            ...
            <section className="content-container flex flex-col justify-center items-center  min-h-screen ">
                <form action="" className='auth-form flex flex-col h-full rounded p-6 bg-white relative' onSubmit={formSubmitHandler}>
                    ...
                    <div className='flex items-center border-2 rounded px-2 my-4'>
                        ...
                        <input type="text" className='bg-inherit w-full outline-none p-2' placeholder='Your name here' value={signUpFormData.fullName} onChange={(event) => setSignUpFormData({ ...signUpFormData, fullName: event.target.value })} required />
                    </div>

                    {/* Username section here */}
                    <div className='flex items-center border-2 rounded px-2 my-4'>
                        <BsKeyFill className='text-xl ' />
                        <input type="text" className='bg-inherit w-full outline-none p-2' placeholder='Username here' value={signUpFormData.username} onChange={(event) => setSignUpFormData({ ...signUpFormData, username: event.target.value })} required />
                    </div>

                    {/* Email section here */}
                    <div className='flex items-center border-2 rounded px-2 my-4'>
                        <MdEmail className='text-xl ' />
                        <input type="email" className='bg-inherit w-full outline-none p-2' placeholder='Email here' value={signUpFormData.email} onChange={(event) => setSignUpFormData({ ...signUpFormData, email: event.target.value })} required />
                    </div>

                    {/* Password section here */}
                    <div className='flex items-center border-2 rounded px-2 my-4'>
                        <RiLockPasswordFill className='text-xl' />
                        <input type={signUpFormData.passType} className='bg-inherit w-full outline-none p-2' placeholder='Password here' value={signUpFormData.password} onChange={(event) => setSignUpFormData({ ...signUpFormData, password: event.target.value })} required />
                        {
