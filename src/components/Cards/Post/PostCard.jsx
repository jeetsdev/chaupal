            ...
            <section className="w-full">
                ...
                <div className="flex justify-between text-xl mt-6">
                        ...
                        <div className="flex justify-center items-center">
                            ...
                            {/* Checking if already liked */}
                            {
                                isAlreadyLikedByUser ?
                                    <BsHeartFill className="text-primary btn-icon bg-inherit hover:cursor-pointer" onClick={dislikeHandler} />
                                    :
                                    <BsHeart className="text-primary btn-icon bg-inherit hover:cursor-pointer" onClick={likeHandler} />
                            }
                    ...
                    <div className="relative">
                        ...
                        {/* Menu button here */}
                        <BsThreeDotsVertical className="w-5 h-5 ml-auto rounded-full text-xl p-px  hover:cursor-pointer hover:text-blue-500 transition-colors" onClick={(event) => {
                            event.stopPropagation();
                            setPostMenu(prev => !prev);
                        }} />

                        {/* Post menu here */}
                        <div className={"post-menu absolute z-0 flex justify-center items-center gap-1 bg--200 -top-1 right-6 rounded " + (postMenu ? "show-menu" : "hide-menu")}>

                            {/* Checking if this is the user post */}
                            {username === post?.username && <div className="flex gap-1">
                                <AiFillEdit className="text-3xl bg-primary rounded hover:cursor-pointer hover:text-blue-500 btn-icon transition-colors p-1" onClick={editHandler} />
                                <AiFillDelete className="text-3xl bg-primary rounded hover:cursor-pointer hover:text-blue-500 btn-icon transition-colors p-1" onClick={deleteHandler} />
                            </div>}

                            {/* Checking of already bookmarked by user */}
                            {
                                isAlreadyBookmarkedByUser ?
                                    <BsBookmarkCheckFill className="text-3xl bg-primary rounded btn-icon hover:cursor-pointer hover:text-blue-500 transition-colors p-1" onClick={bookmarkHandler} />
                                    :
                                    <BsBookmarkPlusFill className="text-3xl bg-primary rounded btn-icon hover:cursor-pointer hover:text-blue-500 transition-colors p-1" onClick={bookmarkHandler} />
                            }
                        </div>
                    </div>
                </div>
